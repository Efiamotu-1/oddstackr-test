        ...
        {users.map((user) => (
          <div key={user.id} className=' md:text-lg grid grid-cols-3 gap-5 items-center p-1 border-b border-stone-300 cursor-pointer'>
            <div className='flex gap-1'>
              ...
              <p className='text-[#CC9200] text'>${user.address.zipcode.split('-')[0]}</p>
            </div>
            <div className='justify-self-center text-[#4D4D4D] text'>{user.address.zipcode.split('-')[0].slice(3)}</div>
            <div className='justify-self-end flex flex-col md:flex-row md:gap-5 items-center text-[#00990F] text font-bold'>
              ...
              <div onClick={() => setShowModal(show => !show)} className='btn font-normal rounded-lg text text-[#EEE5FF] px-4 md:px-8 bg-[#5404FF] flex items-center gap-1'>
                <p>Actions</p>
